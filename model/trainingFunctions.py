...
def train(train_loader, model, criterion, optimizer, epoch, print_freq, plot_data, gpu):
    ...
    for i, (image, object_features, target, id) in enumerate(train_loader):
        ...
        # compute output
        output = model(image_var, object_features_var)
...
def validate(val_loader, model, criterion, print_freq, plot_data, gpu):
    with torch.no_grad():
        ...
        for i, (image, object_features, target) in enumerate(val_loader):
